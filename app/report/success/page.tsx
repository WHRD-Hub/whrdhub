    ...
    <main className="min-h-screen bg-muted/30 flex items-center justify-center px-5 py-16">
      <div className="max-w-md w-full space-y-5">
        ...
        <div className="text-center">
          ...
          <h1 className="text-2xl font-black text-foreground mb-2">{s.title}</h1>
          <p className="text-muted-foreground text-sm leading-relaxed">{s.subtitle}</p>
        </div>

        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm space-y-4">
          <h2 className="font-bold text-sm text-foreground">{s.nextSteps}</h2>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 text-sm">
          <p className="font-semibold text-primary mb-1">{s.alreadySignedIn}</p>
          <p className="text-muted-foreground text-xs">{s.credentialsSub}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button asChild className="w-full">
            <Link href="/dashboard">{s.goToDashboard} <ArrowRight className="w-4 h-4 ml-1" /></Link>
          </Button>
          <Button variant="outline" asChild className="w-full">
            <Link href="/">{s.returnHome}</Link>
          </Button>
        </div>
