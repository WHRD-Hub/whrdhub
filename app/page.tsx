...
import {
  ...
  FileText, ArrowRight, Phone, CheckCircle, LayoutDashboard,
} from "lucide-react";
  ...
  return (
    <main className="min-h-screen flex flex-col bg-background">
      ...
      <section id="how-it-works" className="py-24 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "hsl(27, 87%, 52%)" }}>Simple process</p>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground mb-3">How it works</h2>
            <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">Three steps. No account needed. Your identity is protected at every stage.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { step: "01", icon: <FileText className="w-5 h-5" />, title: "Share what happened", desc: "Describe the incident in your own words. Only share what feels safe. No field forces sensitive details." },
              { step: "02", icon: <Lock className="w-5 h-5" />, title: "Get a private account", desc: "We create an anonymous username for you automatically. No email required to report." },
              { step: "03", icon: <Shield className="w-5 h-5" />, title: "Access support", desc: "Your report reaches WHRD Hub defenders. Receive referrals for legal, medical, and counselling support." },
            ].map((item) => (
              <div key={item.step} className="relative p-7 rounded-2xl border border-border bg-background hover:shadow-md transition-shadow">
                <span className="absolute top-5 right-6 text-5xl font-black text-primary/[0.07] select-none leading-none">{item.step}</span>
                <div className="w-10 h-10 rounded-xl bg-primary text-white flex items-center justify-center mb-5">{item.icon}</div>
      ...
      <section className="py-24 px-6 bg-muted/30 border-y border-border">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "hsl(192, 100%, 38%)" }}>We hear you</p>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground mb-3">What you can report</h2>
            <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">For Women Human Rights Defenders and community members experiencing any of the following.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              ...
              <div key={item.label} className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-border hover:shadow-sm transition-shadow">
                <div className="w-8 h-8 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0">{item.icon}</div>
      ...
      <section className="py-24 px-6 bg-primary text-white">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="text-white/70 mb-8 text-base leading-relaxed">WHRD Hub defenders are here to listen, support, and act. Your report is safe with us.</p>
          <Link href="/report" className="group inline-flex items-center gap-2 bg-white text-primary px-8 py-3.5 rounded-lg font-bold text-sm hover:bg-white/95 transition-colors">
            Report safely
            <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5" />
          </Link>
        </div>
      ...
      <div className="bg-red-50 border-t border-red-200 py-3 px-6">
        <div className="max-w-3xl mx-auto flex flex-wrap items-center justify-center gap-5 text-sm">
          <span className="text-red-700 font-semibold flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" />In immediate danger?</span>
