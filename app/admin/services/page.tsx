  ...
  if (!services?.length) {
    return (
      <div className="bg-white rounded-xl border border-border p-12 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">Add support services using the form above.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {Object.entries(byCategory).map(([cat, svcs]) => (
        <div key={cat}>
          <h3 className="font-bold text-sm uppercase tracking-wide text-muted-foreground mb-3 capitalize">
            {cat.replace(/_/g, " ")} ({svcs.length})
          </h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {svcs.map(s => (
              <div key={s.id} className="bg-white rounded-xl border border-border p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-semibold text-sm">{s.name}</p>
                    {s.organization && <p className="text-xs text-muted-foreground">{s.organization}</p>}
                  </div>
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {s.county && <p className="font-medium text-foreground">📍 {s.county}</p>}
                </div>
...
export default function AdminServicesPage() {
  return (
    <div className="p-8 space-y-8">
      <div>
        <h1 className="text-2xl font-black text-foreground mb-1">Support Services</h1>
        <p className="text-muted-foreground text-sm">Manage services that can be assigned to verified reports.</p>
      </div>
      <AddServiceForm />
      <Suspense fallback={<div className="text-center text-muted-foreground py-8">Loading services...</div>}>
        <ServicesList />
      </Suspense>
    </div>
  );
