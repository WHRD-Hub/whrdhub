...
import { ArrowLeft, ExternalLink, Shield, CheckCircle, XCircle, HelpCircle, MapPin, Calendar, User, AlertTriangle, Briefcase, Plus, Trash2 } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { FactCheckForm } from "@/components/admin/fact-check-form";
import { AssignServiceForm } from "@/components/admin/assign-service-form";
...
function Field({ label, value, mono }: { label: string; value?: string | null; mono?: boolean }) {
  ...
  return (
    <div>
      ...
      <p className={`text-sm ${mono ? "font-mono bg-muted/40 px-2 py-1 rounded" : ""}`}>{value}</p>
    </div>
  );
}
...
async function ReportDetail({ id }: { id: string }) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      {/* Back */}
      <Link href="/admin/reports" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-foreground mb-2">Report Detail</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl border border-border p-6 space-y-5">
            ...
            {report.tfgbv_platform && (
              <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 space-y-2">
                ...
                {report.tfgbv_link && (
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Link / URL</p>
            ...
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase mb-1.5 flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />WHEN</p>
              <p className="text-sm">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-border p-5 space-y-3">
            <h3 className="font-bold text-sm flex items-center gap-2"><User className="w-4 h-4 text-primary" />Reporter</h3>
          ...
          <div className="bg-white rounded-xl border border-border p-5 space-y-4">
            <h3 className="font-bold text-sm flex items-center gap-2"><Briefcase className="w-4 h-4 text-primary" />Assigned Services</h3>
            {!assignedServices?.length ? (
              <p className="text-xs text-muted-foreground">No services assigned yet.</p>
            ) : (
              <div className="space-y-2">
                {assignedServices.map(s => {
                  ...
                  return (
                    <div key={s.id} className="p-3 rounded-lg bg-muted/30 border border-border text-xs space-y-1">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-semibold text-sm">{svc?.name}</p>
                          <p className="text-muted-foreground capitalize">{svc?.category} · {svc?.organization}</p>
                        </div>
                        <form action={async () => {
                          "use server";
                          const { removeService } = await import("@/app/actions/admin-actions");
...
export default function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted-foreground">Loading report...</div>}>
