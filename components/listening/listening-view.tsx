  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-foreground flex items-center gap-2"><RadioTower className="w-6 h-6 text-primary" /> Online Listening</h1>
          <p className="text-sm text-muted-foreground mt-1">Watches your connected Meta assets for abuse keywords and flags them for review.</p>
        </div>
        <button onClick={() => run("poll", pollMeta)} disabled={busy !== null || !connected}
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-primary text-primary-foreground px-4 h-10 text-sm font-bold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {connected ? (
        ...
        <div className="rounded-xl border border-border bg-muted/30 p-5">
          <div className="flex items-start gap-3">
            <span className="w-10 h-10 rounded-xl bg-primary/10 text-primary grid place-items-center shrink-0"><Plug className="w-5 h-5" /></span>
            <div className="min-w-0">
              <h3 className="font-bold text-foreground">Connect Meta to activate listening</h3>
              <p className="text-sm text-muted-foreground mt-0.5">Everything is wired up and waiting on your Meta credentials. You can add and fine-tune the watch keywords below right now; captured signals will appear here the moment it is connected.</p>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <h2 className="font-bold text-foreground flex items-center gap-2"><Search className="w-4 h-4 text-primary" /> Watch keywords</h2>
        <p className="text-xs text-muted-foreground mt-1">Comments and posts containing these words are captured. High-severity words are surfaced first.</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {keywords.map((k) => (
            <span key={k.id} className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${k.active ? SEV[k.severity] ?? SEV.low : "bg-muted text-muted-foreground border-border line-through opacity-60"}`}>
          ...
          {keywords.length === 0 && <p className="text-sm text-muted-foreground">No keywords yet. Add some below.</p>}
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <input value={newWord} onChange={(e) => setNewWord(e.target.value)} placeholder="Add a keyword"
            ...
            className="rounded-lg border border-border bg-background px-3 h-9 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
          <select value={newSev} onChange={(e) => setNewSev(e.target.value)} className="rounded-lg border border-border bg-background px-2 h-9 text-sm">
          ...
          <button onClick={() => run("add", async () => { const r = await addKeyword(newWord, newSev); if (!r?.error) setNewWord(""); return r; })} disabled={busy !== null || !newWord.trim()}
            className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 h-9 text-sm font-semibold hover:bg-muted disabled:opacity-50">
      ...
      {/* Results */}
      <section>
        <div className="flex gap-1 rounded-lg border border-border bg-muted/40 p-1 w-fit mb-4">
          {STATUS_TABS.map((s) => (
            <button key={s} onClick={() => setTab(s)}
              className={`rounded-md px-3 py-1.5 text-sm font-bold capitalize transition-colors ${tab === s ? "bg-card text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        {shown.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
            <RadioTower className="w-8 h-8 mx-auto text-primary/60" />
            ...
            {shown.map((r) => (
              <article key={r.id} className="rounded-xl border border-border bg-card p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-muted-foreground capitalize">{r.source} · {new Date(r.captured_at).toLocaleDateString()}</span>
                </div>
                <p className="mt-2 text-sm text-foreground line-clamp-4">{r.content}</p>
                <div className="mt-2 flex flex-wrap gap-1">
                  {r.matched_keywords.map((w) => <span key={w} className="text-[11px] rounded bg-primary/10 text-primary px-1.5 py-0.5 font-semibold">{w}</span>)}
                </div>
                <div className="mt-3 flex items-center justify-between gap-2 border-t border-border pt-3">
                  ...
                  {r.author && <span className="text-xs text-muted-foreground truncate max-w-[8rem]">{r.author}</span>}
                  {r.permalink && <a href={r.permalink} target="_blank" rel="noopener noreferrer" className="text-primary shrink-0" title="Open on Meta"><ExternalLink className="w-4 h-4" /></a>}
                </div>
              </article>
            ))}
          </div>
...
function StatusBtn({ label, onClick, busy }: { label: string; onClick: () => void; busy: boolean }) {
  return (
    <button onClick={onClick} disabled={busy} className="rounded-md border border-border px-2 py-1 text-[11px] font-semibold text-foreground hover:bg-muted disabled:opacity-50">
