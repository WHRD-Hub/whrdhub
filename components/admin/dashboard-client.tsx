  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-border p-4 flex flex-wrap gap-3 items-end">
        <div>
          <label className="block text-xs font-semibold mb-1 text-muted-foreground">County</label>
          <select value={countyFilter} onChange={e => setCountyFilter(e.target.value)} className="rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
            <option value="">All counties</option>
            {uniqueCounties.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-semibold mb-1 text-muted-foreground">Urgency</label>
          <select value={urgencyFilter} onChange={e => setUrgencyFilter(e.target.value)} className="rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <div>
          <label className="block text-xs font-semibold mb-1 text-muted-foreground">Fact-Check</label>
          <select value={verifFilter} onChange={e => setVerifFilter(e.target.value)} className="rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
            <option value="">All</option>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link href="/admin/reports"><ExternalLink className="w-3.5 h-3.5 mr-1.5" />Full report list</Link>
          </Button>
        </div>
        <div className="bg-white rounded-xl border border-border overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/30 border-b border-border">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Incident</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">County</th>
                  ...
                  <tr key={r.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {(r.incident_types || []).slice(0, 2).map(t => (
                          <span key={t} className="text-xs bg-muted px-1.5 py-0.5 rounded text-muted-foreground capitalize">{t.replace(/_/g, " ")}</span>
                        ))}
                        {(r.incident_types || []).length > 2 && <span className="text-xs text-muted-foreground">+{(r.incident_types || []).length - 2}</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/admin/reports/${r.id}`}><ExternalLink className="w-3.5 h-3.5" /></Link>
                      </Button>
                    </td>
                  </tr>
                ))}
          ...
          {filtered.length > 20 && (
            <div className="px-4 py-3 border-t border-border text-center">
              <Button variant="ghost" size="sm" asChild>
                <Link href="/admin/reports">View all {filtered.length} reports →</Link>
              </Button>
            </div>
          )}
        </div>
