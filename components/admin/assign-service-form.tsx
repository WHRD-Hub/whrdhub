          ...
          <div className="space-y-1.5">
            {remainingSuggested.map(s => (
              <button
                ...
                className="w-full flex items-center justify-between gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-left text-xs hover:bg-primary/10 transition-colors disabled:opacity-60"
              >
                <span className="min-w-0">
                  <span className="font-semibold text-foreground block truncate">{s.name}</span>
                  <span className="text-muted-foreground capitalize">
                    {s.category}{s.organization ? ` · ${s.organization}` : ""}
                  </span>
                </span>
                <span className="shrink-0 inline-flex items-center gap-1 text-primary font-semibold">
                  {pendingId === s.id
