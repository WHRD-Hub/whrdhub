...
export function LinkageGraph({
  ...
  const kpis = [
    { icon: Share2, label: "Total referrals", value: totalReferrals, tint: "bg-primary/10 text-primary" },
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-black text-foreground flex items-center gap-2"><GitBranch className="w-6 h-6 text-primary" /> Referral Linkages</h1>
        <p className="text-sm text-muted-foreground mt-1">How reports connect to the service providers they are referred to, by county and support type.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl border border-border bg-card p-4">
            ...
            <p className="mt-2 text-xl font-black text-foreground truncate">{k.value}</p>
      ...
      {totalReferrals === 0 ? (
        <div className="rounded-xl border border-dashed border-border p-12 text-center">
          <GitBranch className="w-8 h-8 mx-auto text-primary/60" />
          <p className="mt-3 font-semibold text-foreground">No referrals to visualise yet</p>
          <p className="text-sm text-muted-foreground mt-1">Once reports are matched to services, the linkages between counties and providers will appear here.</p>
        </div>
      ) : (
        <>
          {/* Filters */}
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => setCat("all")}
              className={`rounded-full px-3 py-1.5 text-xs font-bold border transition-colors ${cat === "all" ? "bg-primary text-white border-primary" : "border-border text-muted-foreground hover:bg-muted"}`}>
              All support types
            </button>
            {categories.map((c) => (
              <button key={c} onClick={() => setCat(c)}
                className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold border transition-colors ${cat === c ? "text-white border-transparent" : "border-border text-muted-foreground hover:bg-muted"}`}
          ...
          <div className="rounded-xl border border-border bg-card p-3 sm:p-5 overflow-x-auto">
            <div className="flex justify-between text-[11px] font-bold uppercase tracking-wide text-muted-foreground px-1 mb-2" style={{ minWidth: 560 }}>
            ...
            <p className="text-[11px] text-muted-foreground mt-2 px-1">Line thickness shows how many reports flow along each link. Hover a node to isolate its connections.{(counties.length > 14 || services.length > 14) ? " Showing the top 14 on each side." : ""}</p>
          </div>
